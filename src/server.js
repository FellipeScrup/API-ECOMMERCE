...
const {
  generatePromotionsForUser,
  generatePromotionsForAllUsers,
} = require("./utils/promotionGenerator");
...
(async () => {
  try {
    ...
    console.log(
      "Servidor conectado ao banco de dados e pronto para receber solicitações."
    );
      ...
      server = app
        ...
        .on("error", (err) => {
          if (err.code === "EADDRINUSE") {
            console.log(
              `Porta ${port} já está em uso. Tentando porta ${port + 1}...`
            );
            startServer(port + 1);
          } else {
...
app.get("/promocoes/:userId", async (req, res) => {
  try {
    ...
    const promotionsCursor = await r
      .table("promotions")
      .filter({ userId })
  ...
  try {
    ...
    if (!userId || !productId || tempoVisualizacao == null) {
      return res
        .status(400)
        .json({
          error: "userId, productId, e tempoVisualizacao são obrigatórios",
        });
    }
    ...
    res.json({
      message: "Visualização registrada e promoções atualizadas com sucesso",
    });
  } catch (error) {
    console.error("Erro ao registrar visualização e gerar promoções:", error); // Log the detailed error
    res
      .status(500)
      .json({ error: "Erro ao registrar visualização e gerar promoções" });
  }
});
...
app.get("/recomendacoes/:userId", async (req, res) => {
  try {
    ...
    const totalViewTime = userViews.reduce(
      (acc, view) => acc + view.tempoVisualizacao,
      0
    );

    // Get categories and tags from products the user viewed
    const viewedProductsCursor = await r
      .table("products")
    ...
    const categoriesViewed = [
      ...new Set(viewedProducts.map((p) => p.categoria)),
    ];
    const tagsViewed = [...new Set(viewedProducts.flatMap((p) => p.tags))];

    const similarUsersCursor = await r
      .table("views")
      .filter((view) =>
        r
          .expr(viewedProductIds)
          .contains(view("productId"))
          .and(view("userId").ne(userId))
      )
      .pluck("userId")
      .distinct()
      .run(connection);
    ...
    const recommendedViewsCursor = await r
      .table("views")
      .filter((view) =>
        r
          .expr(similarUserIds)
          .contains(view("userId"))
    ...
    const recommendedProductIds = [
      ...new Set(recommendedViews.map((view) => view.productId)),
    ];

    const recommendedProductsCursor = await r
      .table("products")
      .getAll(r.args(recommendedProductIds))
      .filter((product) =>
        r
          .expr(categoriesViewed)
          .contains(product("categoria"))
          .or(
            product("tags").contains((tag) => r.expr(tagsViewed).contains(tag))
          )
      )
      .limit(10)
      .run(connection);
    ...
    res.json({
      ...
      recomendacoes: recommendedProducts.map((p) => ({
        ...
        preco: p.preco,
        motivoRecomendacao: `Usuários similares visualizaram este produto por ${
          productScores[p.id].totalTime
        } segundos no total`,
        score: p.score,
      })),
    });
